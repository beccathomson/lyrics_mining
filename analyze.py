...
#         print("Most correlated terms: " + str(genre_names[code]))
#         print("\n".join(feature_names[-n:]))


def print_predictions(predictions, y_train):
    with open('predictions_file.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(y_train)
        for classifier in predictions:
            row = predictions[classifier]
...
def predict_genre(x, y):
    ...
    print_predictions(predictions, y_train)
...
def main():
    ...
    # print(df.count)# with lowest accuracy
